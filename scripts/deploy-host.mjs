...
// apps/web builds with pnpm through corepack. Launch agents run with a minimal
// PATH, so resolve corepack beside node first and keep a known fallback; the
// pnpm commands receive a PATH that includes node and a writable COREPACK_HOME.
async function corepackBin() {
  const candidates = [
    process.env.COREPACK_BIN,
    path.join(path.dirname(process.execPath), 'corepack'),
    '/Users/a3fckxmini/.hermes/node/bin/corepack',
  ].filter(Boolean);
...
async function poll() {
  ...
  try {
    ...
    if (!await exists(directory)) {
      ...
      const web = path.join(directory, 'apps/web');
      ...
      await command(await corepackBin(), ['pnpm', 'install', '--frozen-lockfile'], web, { env: pnpmEnv() });
      await command(await corepackBin(), ['pnpm', 'build'], web, { env: pnpmEnv() });
...
async function probe(url) {
  try {
    ...
    return { status: response.status, contentType: response.headers.get('content-type'), bytes: text.length };
  } catch (error) { return { error: error.cause?.code || error.message }; }
}
